...
# Parameters
case_study_parameters <- read.csv("Parameters/trial_parameters_new.csv")
...
temp <- seq(10, 30, by = 0.1) # temperature range (Dovlo (2016). Seasonal variation in temperature in the Gulf of Guinea; Idike and Lupo (2024). Analysis of sea surface temperature patterns, vari...(29.34))
...
optimal_temp <- target_species$tempr_opt#27.13 #  source: Dovlo (2016). Seasonal variation in temperature in the Gulf of Guinea;
IPCC_scnrs <- c(1.5, 2, 4) # optimal temperature based on ipcc (CMIP6) projections (source: Sohou et al. 2020; https://www.ipcc.ch/report/ar6/wg1/downloads/factsheets/IPCC_AR6_WGI_Regional_Fact_Sheet_Australasia.pdf)
...
for (irow in 1:n){
  ...
  temp_dev <- temp - optimal_temp[irow]
  ...
  for (i in 1:scnr_temp){
    ...
    k_new_clim <- k_temp_function(temp_dev[i], max_temp_dev, min_temp_dev, optimal_temp_dev, k)
    #make sure K doesn't go less than zero
    k_new_clim <- max(c(0, k_new_clim))

    #update this with linf model... 
    Linf_new_clim <- Linf_temp_function(k, Linf, k_new_clim, temp_dev[i], decline_gro_temp[irow])
    # Linf_new_clim <- Linf_tempr(k, Linf, k_new_clim, phi_t)
    # Linf_new_clim <- Linf_temp_function(k, Linf, k_new_clim, temp_dev[i], target_species$dec_gr[irow])
