...
tempr <- seq(22.52, 29.34, by = 0.1) # temperature range (Dovlo (2016). Seasoaal variation in temperature in the Gulf of Guinea; Idike and Lupo (2024). Analysis of sea surface temperature patterns, vari...)
tempr_min_dev <- -6.5 # minimum temperature deviation from optimal temperature    
tempr_max_dev <- 6.5 # maximum temperature deviation from optimal temperature   
tempr_opt <- 27.13 #  source: Dovlo (2016). Seasoaal variation in temperature in the Gulf of Guinea;
...
tempr_diff <- tempr - tempr_opt
tempr_opt_diff <- tempr_opt - tempr_opt
 
k_opt <- 0.3 # optimal growth coefficient (source: fishbase.se/manual/key%20facts.htm)
scnr_tempr <- length(tempr_diff)
...
for (irow in 1:n){
  ...
  scnr_clim <- expand.grid(tempr_diff = tempr_diff, Fmort = target_species$Fmort.[irow], target_species = target_species$species[irow])
  ...
  for (i in 1:scnr_tempr){
    ...
    k_new_clim <- k_tempr(tempr_diff[i], tempr_max_dev, tempr_min_dev, tempr_opt_diff, k_opt)
...
g3 <- ggplot(all_scnr_data) + 
      aes(x = tempr_diff, y = size_indicator_Linf, color = Fmort, group = Fmort) +
      geom_line() +
      facet_wrap(~target_species) + 
      geom_vline(xintercept = tempr_opt_diff, linetype = 2) +
      geom_vline(xintercept = IPCC_scnrs, linetype = 2, col = "grey") +
      geom_hline(yintercept = k_opt, linetype = 2) +
      annotate("text", x = tempr_opt_diff, y = max(all_scnr_data$size_indicator_Linf), 
